...
router.get("/:jobId", async (req, res) => {
  try {
    ...
    const { rows: clips } = await query(
      `SELECT c.id, c.title, c.file_url, c.cloud_url, c.file_path, c.duration, c.start_time, c.end_time,
...
router.get("/library/all", async (req, res) => {
  ...
  try {
    ...
    const { rows: clips } = await query(
      `SELECT c.id, c.job_id, c.title, c.file_url, c.cloud_url, c.file_path, c.duration,
              c.hook_score, c.platforms, c.thumbnail_color, c.thumbnail_url,
              c.transcript, c.created_at
       FROM clips c
       ${whereClause}
